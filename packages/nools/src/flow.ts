...
import { LinkedList } from "./STD/linked-list";
...
export class Flow extends EventEmitter<AgendaGroupEvent | FlowEvent> {
  ...
  getFacts(Type: any) {
    return Type
      ? this.workingMemory.getFactsByType(Type)
      : this.workingMemory.getFacts();
  }
  ...
  assert<TObject extends FactObject>(fact: TObject) {
    this.rootNode.assertFact(
      this.data,
      this.workingMemory.assertFact(fact),
      this.workingMemory
    );
  ...
  retract<TObject extends FactObject>(fact: TObject) {
    ...
    this.rootNode.retractFact(
      this.data,
      this.workingMemory.retractFact(fact),
      this.workingMemory
    );
  ...
  modify(fact: any) {
    //fact = this.workingMemory.getFact(fact);
    this.rootNode.modifyFact(
      this.data,
      this.workingMemory.modifyFact(fact),
      this.workingMemory
    );
