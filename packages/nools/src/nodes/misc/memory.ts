...
export function Memory(): IMemory {
  return {
    length: 0,
    head: null,
    tail: null,
...
export function memory_push(m: IMemory, data: Context): ITuple {
  const tail = m.tail,
    head = m.head,
    node: ITuple = {
      data: data,
...
export function memory_remove(m: IMemory, node: ITuple): void {
  if (node.prev) {
...
export function memory_clear(m: IMemory): void {
  m.head = m.tail = null;
  m.length = 0;
  clearIndexes(m);
}

function clearIndexes(m: IMemory): void {
  m.tables.reset();
  m.indexes.length = 0;
}

function __index(m: IMemory, node: ITuple): void {
  ...
  let currEntry: Table;
  let prevLookup: Table | undefined = undefined;
  while (++i < l) {
...
function __removeFromIndex(m: IMemory, node: ITuple): void {
...
export function memory_get(m: IMemory, tuple: Context): ITuple[] {
  let ret: ITuple[];
  if (!m.length) {
    ret = [];
  } else {
    ret = getMemory(m.tables, tuple.factHash, m.indexes);
  }
  return ret;
}

function __createIndexTree(m: IMemory): void {
