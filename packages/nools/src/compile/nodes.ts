...
import { IBucket, IRule } from "../interfaces";
...
function __mergeJoinNodes(root: IRootNode) {
  ...
  for (let i = 0; i < joinNodes.length; i++) {
    ...
    if (
      ...
      j1.constraint &&
      j2.constraint &&
      j1.constraint.constraint.equal(j2.constraint.constraint)
    ) {
      merge(jj1, jj2, nodes);
...
function _create_join_node(type: nodeType): IJoinNode {
  ...
  const constraint = create_join_reference_node(
    node.leftTuples,
    node.rightTuples
  );
  return mixin(node, {
...
function _create_from_not_node(
  root: IRootNode,
  type: nodeType,
  pattern: IFromPattern
): IFromNotNode {
...
function create_exists_from_node(
  root: IRootNode,
  pattern: IFromPattern
): IExistsFromNode {
...
function __createJoinNode(
  root: IRootNode,
  rule: number,
  pattern: ICompositePattern,
  out_node: number,
  side: Side
) {
  ...
  if (right_type === patternType.not) {
    ...
    joinNode = create_exists_from_node(
      root,
      pattern.rightPattern as IFromPattern
    );
    // @ts-ignore
    jn = nodes.push(joinNode) - 1;
...
function __addToNetwork(
  ...
  if (type === patternType.composite) {
    ...
    __createBetaNode(
      root,
      rule,
      composite_pattern(initial_fact_pattern(root.cs), pattern),
      outNode,
      side
    );
  } else {
...
function __createAlphaNode(
  ...
  const type = pattern.tp;
  if (
    type !== patternType.from &&
    type !== patternType.from_exists &&
    type !== patternType.from_not
  ) {
    const nodes = root.ns;
    const outNode = nodes[out_node];
    ...
    constraints
      ...
      .forEach((c) => {
        ...
        if (is_instance_of_hash(constraint)) {
          n = __createPropertyNode(
            root,
            rule,
            constraint as IHashConstraint,
            c
          );
          ...
          n = __createEqualityNode(
            root,
            rule,
            constraint as IObjectConstraint,
            c
          );
        }
        const node = nodes[n];
